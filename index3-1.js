...
const dbWriter = require('./core/dbWriter');
...
class PC28Bot {
  ...
  async placeBet(period) {
    try {
      ...
      if (betting.placeBet(period, bet)) {
        ...
        await TelegramBetting.sendBet(period, bet);
  ...
  // 等待开奖结果
  handleResultWaiting() {
    logger.info(`开始等待第${this.currentPeriod}期开奖结果...`);
    this.clearTimers();

    // 如果已有监听器，不再重复添加
    if (this.resultHandlers.has(this.currentPeriod)) {
      return;
    }

    const resultHandler = (result) => {
      if (result.period === this.currentPeriod.toString()) {
        this.handleNewResult(result);
      }
    };

    this.resultHandlers.set(this.currentPeriod, resultHandler);
    lottery.addResultListener(resultHandler);
  }

  // 处理新结果
  handleNewResult(result) {
    // if (betting.isProcessed(result.period)) {
    //   logger.warn(`第 ${result.period} 期投注已处理过，跳过`);
    //   return;
    // }
    this.clearTimers();
    ...
    // index3-1.js
    // index3-1.js
    // dbWriter.writeResult(result)
    // .then(writeResult => {
    //     if (writeResult.success) {
    //         logger.success(`第 ${writeResult.period} 期写入成功`);
    //     } else {
    //         logger.error(`写入失败: ${writeResult.error}`);
    //     }
    // })
    // .catch(err => {
    //     logger.error('系统级错误:', err);
    // });
...
async function bootstrap() {
  try {
    // 第一步：数据库初始化
    await dbWriter.init();
    logger.success('[启动阶段] 数据库初始化完成');

    // 第二步：Telegram连接
    await telegram.login();
    logger.success('[启动阶段] Telegram连接成功');

    // 第三步：数据同步
    await lottery.fetchRecentResults();
    logger.info(`[启动阶段] 已加载${lottery.results.length}条历史数据`);

    // 第四步：启动核心逻辑
    await new PC28Bot().safeInit();
  } catch (err) {
